...
@section('content')
                            ...
                            <div class="QA_section">
                              <div class="white_box_tittle list_header align-items-end">
                                <div class="d-flex gap-5">
                                  <div>
                                    ...
                                    <p>Tailor: <strong>{{$tailor->name}}</strong></p>
                                    <p>Total Debit: <strong>{{$tailor->accountSummary['debit']}}</strong></p>
                                    <p>Total Credit: <strong>{{$tailor->accountSummary['credit']}}</strong></p>
                                    <p>Total Remaining: <strong>{{$tailor->accountSummary['remaining']}}</strong></p>
                                  </div>
                                  <div>
                              ...
                              <div class="QA_table mb_30">
                                <table class="table lms_table_active">
                                  ...
                                  <tbody>
                                    {{-- @foreach ($tailor->sales as $key => $sale)
                                      <tr>
                                        <td>{{$key+1}}</td>
                                        <th scope="row">{{$sale->created_at->format("m-d-Y")}}</th>
                                        <td>Sale Order</td>
                                        <td>{{$sale->bill_no}}</td>
                                    ...
                                    @foreach ($tailor->ledgers as $key => $ledger)
                                      @php
                                        $debit = 0; $credit = 0;
    ...
    <div class="modal" id="addLedgerModal" tabindex="-1" aria-labelledby="addLedgerModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-md">
          <form action="{{route('tailor.ledger.store',$tailor->id)}}" method="post" id="addLedgerForm">
            @csrf
